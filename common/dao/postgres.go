...
select table_catalog as database,
 ...
 where table_catalog = ?
   and table_schema = ? `)
	...
	if err := gormx.GetInstance(source).Raw(sql.String(), database, schema).Scan(&result).Error; err != nil {
...
select t1.column_name as name,
  ...
  left join pg_description t5 on t5.objoid = t4.attrelid and t5.objsubid = t4.attnum`)
	...
	if err := gormx.GetInstance(source).Raw(sql.String()).Scan(&result).Error; err != nil {
